...
class HubTest(unittest.TestCase):
    def setUp(self):
        ...
        make_cbz(self.lib / "Berserk" / "Berserk v02.cbz", panels={"bubbles": "m", "growth": 3, "pages": {}})
...
class PiecesTest(unittest.TestCase):
    ...
    def test_add_panels_entry_keeps_pages_and_skips_a_changed_file(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertTrue(add_panels_entry(f, b'{"bubbles": "m", "growth": 3, "pages": {}}', file_version(f), FileGate().replace))
            with zipfile.ZipFile(f) as z:
...
(out / (Path(args[0]).stem + ".panels.json")).write_text(json.dumps({"bubbles": "test", "growth": 3, "pages": {}}))
'''
        with tempfile.TemporaryDirectory() as d:
            d = Path(d)
    ...
    def test_read_panels_state(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            make_cbz(d / "c.cbz", panels={"bubbles": "m", "growth": 3, "pages": {}})
